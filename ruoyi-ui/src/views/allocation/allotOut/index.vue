<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="源仓库" prop="warehouseCode">
        <el-select v-model="queryParams.warehouseCode" placeholder="请选择源仓库" clearable>
          <el-option
      ...
      <el-form-item label="目标仓库" prop="destWarehouseCode">
        <el-select v-model="queryParams.destWarehouseCode" placeholder="请选择目标仓库" clearable>
          <el-option
            v-for="item in warehouseList"
            :key="item.warehouseCode"
            :label="item.warehouseName"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['stock:allotOut:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['stock:allotOut:export']"
        >导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="180">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['stock:allotOut:query']"
          >详情</el-button>
          <el-button
            size="mini"
            type="text"
    ...
    <!-- 调拨出库单详情对话框 -->
    <el-dialog class="detail-dialog" title="调拨出库单详情" :visible.sync="detailOpen" width="1200px" append-to-body :close-on-click-modal="false">
        ...
        <el-row>
          <el-col :span="6">
            <el-form-item label="目标仓库：">
              <span>{{detailForm.destWarehouseName}}</span>
            </el-form-item>
          </el-col>
          <el-col :span="6">
      ...
      <el-table class="detail-table" :data="detailList" style="width: 100%">
        ...
        <el-table-column label="已出库数量" align="center" prop="receivedQuantity" width="100" />
...
<script>
import { listAllotOutOrder, getAllotOutOrder, delAllotOutOrder, printAllotOutOrder } from "@/api/allocation/allotOut";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listAllotOutOrder(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleDetail(row) {
      this.detailForm = {};
      this.detailList = [];
      getAllotOutOrder(row.orderId).then(response => {
    ...
    /** 确认打印 */
    confirmPrint() {
      printAllotOutOrder(this.detailForm.orderId).then(response => {
        const binaryData = [];
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除调拨出库单单号为"' + orderNoStr + '"的数据项？').then(function() {
        return delAllotOutOrder(orderIds);
      }).then(() => {
        this.getList();
    ...
    handleExport() {
      this.download('stock/outOrder/export', {
        ...this.queryParams,
        orderType: 'allot'
      }, `allot_out_order_${new Date().getTime()}.xlsx`);
    }
  }
};
</script>
