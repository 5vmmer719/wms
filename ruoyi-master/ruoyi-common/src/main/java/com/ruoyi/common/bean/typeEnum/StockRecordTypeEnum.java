...
public enum StockRecordTypeEnum {
    // 入库
    IN_PURCHASE("in_purchase", "原料入库"),
    IN_OUTSOURCING("in_outsourcing","外协入库"),
    IN_PRODUCTION("in_production", "车间入库"),
    // 出库
    OUT_PRODUCTION("out_production", "生产领料"),
    OUT_REPAIR("out_repair", "补领出库"),
    OUT_COMMON("out_common", "销售出库"),
    // 退货
    IN_PURCHASE_RETURN("in_purchase_return", "原料入库退货"),
    OUT_PRODUCTION_RETURN("out_production_return", "生产领料退货"),
    OUT_REPAIR_RETURN("out_repair_return", "补领出库退货"),
    OUT_COMMON_RETURN("out_common_return", "销售出库退货"),
    // 上下架
    UPPER("upper", "上架"),
    LOWER("lower", "下架"),
    //调拨
